...
fn check_around(data: &Vec<Vec<char>>, n: usize, m_start: usize, m_end: usize) -> bool {
    ...
    for n_lookup in n.saturating_sub(1)..=n.saturating_add(1) {
        for m_lookup in m_start.saturating_sub(1)..=m_end.saturating_add(1) {
            if check_position(data, n_lookup, m_lookup) { total_symbols += 1}
            if total_symbols >= REQUIRED_SYMBOLS { return true}
        }
    }
    false
}
...
fn main() {
    ...
    let data: Vec<Vec<char>> = include_str!("../input2.txt").lines().map(|x| x.chars().collect::<Vec<char>>()).collect();
    ...
    for (n, i) in data.iter().enumerate() {
        ...
        while let Some((m, j)) = iterator.next() {
            if j.is_numeric() {
                ...
                while data.get(n).unwrap().get(number_end).unwrap_or(&'a').is_numeric() {
                    number_end += 1;
                    iterator.next();
                }

                if check_around(&data, n, m, number_end - 1) {
                    let number = data[n][m..number_end].iter().collect::<String>().parse::<i32>().unwrap();
                    a += number;
                }
                else {
                    let number = data[n][m..number_end].iter().collect::<String>().parse::<i32>().unwrap();
